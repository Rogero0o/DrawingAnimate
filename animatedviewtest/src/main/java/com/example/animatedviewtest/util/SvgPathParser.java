...
class SvgPathParser {
    ...
    public Path parsePath(String s) throws ParseException {
        ...
        while (mIndex < mLength) {
            ...
            boolean relative = (mCurrentToken == TOKEN_RELATIVE_COMMAND);//是否是相对位置
            switch (command) {
                case 'M':
                case 'm': {
                    // move command
                    ...
                    while (advanceToNextToken() == TOKEN_VALUE) {
                        ...
                        if (firstPoint) {
                            p.moveTo(tempPoint1.x, tempPoint1.y);//移动到开始位置
                            firstPoint = false;
                            if (firstMove) {
                                mCurrentPoint.set(tempPoint1);
                                firstMove = false;
                            }
                            p.lineTo(tempPoint1.x, tempPoint1.y);//在开始位置画一个点
                        } else {
                        }
                    }
                    mCurrentPoint.set(tempPoint1);
